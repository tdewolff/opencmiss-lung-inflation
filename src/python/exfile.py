...
class ExnodeSection(object):
    ...
    def _calc_num_node_values(self):
        ...
        for field in self.fields:
            for component in field.components:
                num_values += (1 + component.num_derivatives) * component.num_versions
        return num_values
...
class ExnodeComponent(object):
    ...
    def __init__(self, f, field):
        ...
        try:
            self.num_versions = int(read_string_regex(f, declaration[1],
                r'#Versions\s*=\s*([0-9]+)'))
        except ExfileError:
            self.num_versions = 1
